...
subtitle: "성남시, 인구통계: 2016 vs 2020"
author:
...
# 인구수 변화 {#sungnam-demogrpahics}

## 성남시 인구변화 {#sungnam-demogrpahics-sn}

```{r sungnam-EDA-sungnam}
sn_demo %>% 
  group_by(연도) %>% 
...
## 성남3구 인구변화 {#sungnam-demogrpahics-gu}

```{r sungnam-EDA-sungnam-gu}
sn_demo %>% 
  group_by(연도, 구명) %>% 
  summarise(인구수 = sum(인구수)) %>% 
  spread(연도, 인구수) %>% 
...
## 동별 인구변화 {#sungnam-demogrpahics-gu-dong}

```{r sungnam-EDA-sungnam-gu-dong}
sn_dong_demo <- sn_demo %>% 
  group_by(연도, 구명, 동명) %>% 
  summarise(인구수 = sum(인구수)) %>% 
  spread(연도, 인구수) %>% 
  mutate(인구변화 = `2019년` - `2016년` ) %>% 
...
## 동별 인구변화: 수정구 {#sungnam-demogrpahics-gu-dong-sujung}

```{r sungnam-EDA-sungnam-gu-dong-sujung}
sn_dong_demo %>% 
  filter(구명 == "수정구") %>% 
  arrange(desc(변화율)) %>% 
  DT::datatable() %>% 
...
## 동별 인구변화: 중원구 {#sungnam-demogrpahics-gu-dong-jungwon}

```{r sungnam-EDA-sungnam-gu-dong-jungwon}
sn_dong_demo %>% 
  filter(구명 == "중원구") %>% 
  arrange(desc(변화율)) %>% 
  DT::datatable() %>% 
...
# 연령별 {#sungnam-demogrpahics-age}


```{r sungnam-age-distribution}
sn_demo %>% 
  ggplot(aes(x=연령, y=인구수, color = 연도)) +
    geom_point() +
    facet_wrap(~구명) +
