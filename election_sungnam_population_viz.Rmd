...
# 분당을 인터랙티브 시각화 - `rbokeh` {#sungnam-trelliscope-rbokeh}

## "구미동" 2016년 대비 2019년 인구수 변화 {#sungnam-trelliscope-rbokeh-dong}
...
# 연도별, 행정동 중첩시킴.
<<<<<<< HEAD
by_year_dong <- bundang_eul_comp_df %>%
  group_by(`행정동`) %>%
=======
by_year_dong <- bundang_eul_df %>%
  dplyr::mutate(연도 = factor(연도)) %>%
  group_by(`연도`, `행정동`) %>%
>>>>>>> 57db0b8da96890f436de6ed756a0367d1eeb8803
  nest()
...
=======
    ~ figure(xlab="", ylab="") %>% 
      ly_bar(`연령대`, `인구수`,  position = "dodge", data = .x, hover = TRUE) %>% 
      theme_axis("x", major_label_orientation = 90)
      # theme_axis("x", major_label_orientation = 90) %>%  
