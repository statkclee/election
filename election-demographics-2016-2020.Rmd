...
subtitle: "분당을, 인구통계: 2016 vs 2020"
author:
...
# 분당을 변화 {#level-eul-demogrpahics}

## 분당을 인구변화 {#level-eul-demogrpahics-sn}
...
## 동별 인구변화: 분당을 {#eul-demogrpahics-gu-dong-bundang}

```{r eul-EDA-sungnam-gu-dong-bundang}
eul_demo %>% 
  group_by(연도, 동명) %>% 
  summarise(인구수 = sum(인구수)) %>% 
  spread(연도, 인구수) %>% 
...
## (비)유권자: 동별 {#eul-demogrpahics-voter-precinct-dong}

```{r eul-age-voter-precinct-dong}
eul_demo %>% 
...
## 연령: 분당을 동별 {#eul-demogrpahics-age-dong}

```{r eul-age-precinct-dong}
eul_demo %>% 
  filter(연령 >=19) %>% 
  mutate(연령대 = case_when(연령 >= 19 & 연령 <= 29 ~ "20대",
                            연령 >= 30 & 연령 <= 39 ~ "30대",
...
# 시각화 {#eul-demogrpahics-age-dong-viz}

## 연령별 {#eul-demogrpahics-age-dong-viz-age}
...
## 동별 {#eul-demogrpahics-age-dong-viz-dong}

```{r show-difference-dong}
eul_demo %>% 
  filter(연령 >=19) %>% 
  mutate(연령대 = case_when(연령 >= 19 & 연령 <= 29 ~ "20대",
                            연령 >= 30 & 연령 <= 39 ~ "30대",
